...
{
    $dbhost = "127.0.0.1";
    $dbuser = "pascalhnguyen";                       // Your Cloud 9 username
...
$app->get('/clicker/login/{student_id}/{access_code}', function ($req, $res, $args) {

    try 
    {
        $db = getDB();
        $sql = $db->prepare("SELECT 'success' FROM clicker_student WHERE id = :id and access = :pc") ;
 
        $sql->bindParam(':id', $args['student_id'], PDO::PARAM_INT);
        $sql->bindParam(':pc', $args['access_code'], PDO::PARAM_INT);
        $sql->execute();
